...
public class DataInsertionQueries {
	...
	public static final String INSERT_USER = "INSERT INTO users (email, hashedpassword, salt) " + "VALUES (?, ?, ?) ;";
	public static final String INSERT_ROUNDABOUT = "INSERT INTO roundabout (sim_id, roundabout_id, edges, nodes) "
			+ "    (SELECT ?, xml.roundabout_id, xml.edges, xml.nodes " + "     FROM files, "
			+ "          xmltable('/edges/roundabout' PASSING xmlparse(DOCUMENT files.edges) COLUMNS "
			+ "              roundabout_id FOR ORDINALITY, " + "              edges TEXT PATH './@edges' NOT NULL, "
			+ "              nodes TEXT PATH './@nodes' " + "              ) AS xml " + "        WHERE id = ?);";

	public static final String INSERT_ROUTE = "INSERT INTO route (sim_id, vehicle_id, depart, edges) "
			+ "    (SELECT ?, xml.id, xml.depart, xml.edges " + "     FROM files, "
			...
			+ "              edges TEXT PATH './route/@edges' NOT NULL " + "              ) AS xml "
			+ "        WHERE files.id = ?);";
	...
	public static final String INSERT_OUTPUT = "INSERT INTO output (sim_id, time_step, vehicle_id, x, y, angle, type, speed, pos, slope, lane) "
			...
			+ "              type TEXT PATH './@type' NOT NULL, " + "              x NUMERIC PATH './@x' NOT NULL, "
			...
			+ "              pos NUMERIC PATH './@pos' NOT NULL, " + "              lane TEXT PATH './@lane' NOT NULL, "
			+ "              slope NUMERIC PATH './@slope' NOT NULL " + "              ) AS xml "
			+ "        WHERE id = ?);";

	private static final String INSERT_NODE = "CREATE VIEW node_view AS " + "SELECT xml.* " + "FROM files, "
			+ "     xmltable('/nodes/node' PASSING xmlparse(DOCUMENT nodes) COLUMNS "
			+ "         id TEXT PATH './@id' NOT NULL, " + "         x NUMERIC PATH './@x', "
			+ "         y NUMERIC PATH './@y', " + "         type TEXT PATH './@type', "
			+ "         tl TEXT PATH './@tl', " + "         key TEXT PATH './param/@key', "
			+ "         value TEXT PATH './param/@value' " + "         ) AS xml " + "WHERE files.id = %d; " + " "
			+ "INSERT INTO node (sim_id, node_id, x, y, type) "
			+ "        (SELECT ?, nv.id, nv.x, nv.y, nv.type FROM node_view nv); " + " "
			+ "INSERT INTO node_tl (sim_id, node_id, tl_id) " + "    (SELECT ?, nv.id, nv.tl "
			+ "     FROM node_view nv " + "     WHERE tl IS NOT NULL); " + " "
			+ "INSERT INTO node_data (sim_id, node_id, key, value) " + "    (SELECT ?, nv.id, nv.key, nv.value "
			+ "     FROM node_view nv " + "     WHERE nv.key IS NOT NULL " + "        OR nv.value IS NOT NULL); " + " "
			+ "DROP VIEW IF EXISTS node_view;";

	private static final String INSERT_EDGE = "CREATE VIEW edge_view AS " + "SELECT xml.* " + "FROM files, "
			+ "     xmltable('/edges/edge' PASSING xmlparse(DOCUMENT edges) COLUMNS "
			+ "         edge_id TEXT PATH './@id' NOT NULL, " + "         start NUMERIC PATH './@from' NOT NULL, "
			+ "         finish NUMERIC PATH './@to' NOT NULL, " + "         priority NUMERIC PATH './@priority', "
			+ "         type TEXT PATH './@type' NOT NULL, "
			+ "         num_lanes NUMERIC PATH './@numLanes' NOT NULL, "
			+ "         speed NUMERIC PATH './@speed' NOT NULL, " + "         shape TEXT PATH './@shape', "
			+ "         spread_type TEXT PATH './@spreadType', " + "         length NUMERIC PATH './@length', "
			+ "         width NUMERIC PATH './@width', " + "         name TEXT PATH './@name', "
			+ "         allow TEXT PATH './@allow', " + "         disallow TEXT PATH './@disallow', "
			+ "         key TEXT PATH './param/@key', " + "         value TEXT PATH './param/@value' "
			+ "         ) AS xml " + "WHERE id = %d; " + " "
			+ "INSERT INTO edge (sim_id, edge_id, start, finish, priority, type, num_lanes, speed, shape, spread_type, length, width, "
			+ "                  name, allow, disallow) " + "    (SELECT ?, "
			...
			+ "     FROM edge_view ev); " + " " + "INSERT INTO edge_data (sim_id, edge_id, key, value) "
			+ "    (SELECT ?, ev.edge_id, ev.key, ev.value " + "     FROM edge_view ev "
			+ "     WHERE ev.key IS NOT NULL " + "        OR ev.value IS NOT NULL); " + " " + "INSERT INTO edge_lane "
			+ "    (SELECT ?, xml.edge_id, xml.line_id, xml.acceleration,  "
			+ "            xml.allow, xml.disallow, xml.speed, xml.width " + "     FROM files, "
			+ "          xmltable('//lane' PASSING xmlparse(DOCUMENT files.edges) COLUMNS "
			+ "              edge_id TEXT PATH './../@id' NOT NULL, "
			+ "              line_id INT PATH './@index' NOT NULL, "
			+ "              acceleration INT PATH './@acceleration', " + "              allow TEXT PATH './@allow', "
			+ "              disallow TEXT PATH './@disallow', " + "              speed NUMERIC PATH './@speed', "
			+ "              width NUMERIC PATH './@width' " + "              ) AS xml "
			+ "        WHERE files.id = ?); " + " " + "DROP VIEW IF EXISTS edge_view;";

	public static final String INSERT_SIMULATION = "INSERT INTO simulation (name, description) " + "VALUES (?, ?) "
			+ "RETURNING id;";

	public static final String DROP_VIEWS = "DROP VIEW IF EXISTS node_view; " + "DROP VIEW IF EXISTS edge_view;";
