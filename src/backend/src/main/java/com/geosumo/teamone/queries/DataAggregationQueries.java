...
public class DataAggregationQueries {
	public static final String GET_SIMULATIONS = "SELECT COALESCE(to_jsonb(array_agg(jsonobj.simdata)), '[]') "
			+ "FROM simulation s, "
			...
			+ "              FROM output o " + "              WHERE o.sim_id = s.id) jsonobj;";

	public static final String GET_SIMULATION_BY_ID = "SELECT COALESCE(jsonb_build_object('id', s.id, "
			+ "                          'name', s.name, " + "                          'date', s.upload_date, "
			+ "                          'description', s.description, "
			+ "                          'steps', count(DISTINCT o.time_step)), '[]') " + "FROM simulation s, "
			+ "     output o " + "WHERE s.id = ? " + "  AND o.sim_id = ? " + "GROUP BY s.id;";
	...
	public static final String GET_ALL_NODES = "SELECT COALESCE(to_jsonb(array_agg(nodes.data)), '[]') " + "FROM node, "
			+ "     LATERAL ( SELECT jsonb_build_object('id', node_id, 'lon', x, 'lat', y) AS data) nodes "
			+ "WHERE sim_id = ?";

	public static final String GET_ALL_EDGES = "SELECT COALESCE(jsonb_agg(ultimateData.line), '[]')                    \n"
			+ "FROM (                                         \n"
	...
	public static final String GET_ALL_EDGES_WITH_COUNT = "" + "SELECT COALESCE(jsonb_agg(ultimatedata), '[]')\n"
	...
	public static final String VEHICLE_PER_TIME_STEP = "" + "SELECT COALESCE(jsonb_agg(data), '[]') "
			+ "            FROM ( SELECT time_step, count(*) AS vehicles " + "                    FROM output "
			+ "                    WHERE sim_id = ? " + "                    GROUP BY time_step"
			+ "                    ORDER BY time_step) AS data;";

	public static final String SPEED_PER_TIME_STEP = "SELECT COALESCE(jsonb_agg(data), '[]') "
			+ "                    FROM ( " + "                    SELECT time_step, AVG(speed) as averagespeed "
			+ "                    FROM output " + "                    WHERE sim_id = ? "
			+ "                    GROUP BY time_step" + "                    ORDER BY time_step) as data;";
	...
	public static final String BUSIEST_ROADS_PER_TIME_STEP = "SELECT COALESCE(jsonb_agg(data), '[]')                         \n"
			+ "FROM(                                  \n"
